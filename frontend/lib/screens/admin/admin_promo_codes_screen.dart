...
class _AdminPromoCodesScreenState extends State<AdminPromoCodesScreen> {
  ...
  Future<void> _loadPromoCodes() async {
    ...
    try {
      ...
      if (response.success) {
        setState(() {
          _promoCodes = List<Map<String, dynamic>>.from(
            response.data['data'] ?? [],
          );
          _isLoading = false;
        });
  ...
  Widget _buildContent() {
    ...
    final totalUsage = _promoCodes.fold<int>(
      0,
      (sum, p) => sum + ((p['usageCount'] as int?) ?? 0),
    );
                  ...
                  Expanded(
                    child: FilledButton(
                      ...
                      child: _isSaving
                          ...
                          : Text(
                              _isEditing ? 'Save Changes' : 'Create Code',
                              style: const TextStyle(
                                fontWeight: FontWeight.w600,
                              ),
                            ),
