...
class _AdminCustomersScreenState extends State<AdminCustomersScreen> {
    ...
    return all.where((c) {
      if (_nameFilterCtrl.text.isNotEmpty &&
          !c.fullName.toLowerCase().contains(
            _nameFilterCtrl.text.toLowerCase(),
          )) {
        return false;
      }
      if (_emailFilterCtrl.text.isNotEmpty &&
          !c.email.toLowerCase().contains(
            _emailFilterCtrl.text.toLowerCase(),
          )) {
        return false;
      }
      if (_phoneFilterCtrl.text.isNotEmpty) {
        final phone = c.phone ?? '';
        if (!phone.toLowerCase().contains(
          _phoneFilterCtrl.text.toLowerCase(),
        )) {
          return false;
        }
      }
  ...
  Future<void> _createCustomer() async {
    ...
    try {
      ...
      if (response.generatedPassword != null) {
        ...
        _showSnack(
          'Customer "${response.fullName}" created successfully',
          _success,
        );
      }
      _loadCustomers();
  ...
  Widget _buildContent() {
    ...
    final activeCount = (_customersData?.items ?? [])
        .where((c) => c.isActive)
        .length;
    final withOrders = (_customersData?.items ?? [])
        .where((c) => c.ordersCount > 0)
        .length;
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(10),
          border: Border.all(
            color: isActive ? _indigo.withOpacity(0.3) : Colors.grey.shade200,
          ),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
                  ...
                  child: Row(
                    children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(
                          fontSize: 12,
                          color: e.key == value ? _accent : Colors.grey[600],
                        ),
                      ),
                    ],
                  ),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Icon(
          icon,
          size: 18,
          color: onTap != null ? _accent : Colors.grey[300],
        ),
      ),
    );
  }
