  ...
  private async resolveCategoryImageUrls(
    ...
    if (withImage.length > 0) {
      ...
      for (const c of withImage) {
        const url = this.resolveMediaId(c.image_id!, assetKeyMap);
    ...
    if (withoutImage.length > 0) {
      ...
      // Manually take first product per category
      const catFirstMediaId = new Map<number, string>();
      for (const p of products) {
        const mediaId = p.images[0]?.media_asset_id;
        if (p.categoryId && mediaId && !catFirstMediaId.has(p.categoryId)) {
          catFirstMediaId.set(p.categoryId, mediaId);
        }
      }

      if (catFirstMediaId.size > 0) {
        // Collect UUIDs that need lookup (not direct URLs or paths)
        const uuids = [...catFirstMediaId.values()].filter(
          m => !m.startsWith('http') && !m.includes('/'),
        );
        const assetKeyMap = new Map<string, string>();
        if (uuids.length > 0) {
        ...
        for (const [catId, mediaId] of catFirstMediaId) {
          const url = this.resolveMediaId(mediaId, assetKeyMap);
          if (url) imageMap.set(catId, url);
        }
      }
    }
...
console.log('✓ brands.service.ts written (v2 - handles URLs and UUIDs)');
...
console.log('✓ categories.service.ts written (v2 - handles URLs and UUIDs)');
